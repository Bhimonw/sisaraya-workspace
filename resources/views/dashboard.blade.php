                        ...
                        <div class="p-4 border-b border-gray-200">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    ...
                                    <h4 class="text-base font-semibold text-gray-800">Anggota Tersedia</h4>
                                </div>
                                <span class="px-2 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded-full">
                                    {{ $freeUsersToday->count() }} Free
                                </span>
                            </div>
                        </div>
                        <div class="p-4">
                            <p class="text-xs text-gray-500 mb-3">{{ $today->locale('id')->isoFormat('dddd, D MMMM YYYY') }}</p>
                            
                            @if($freeUsersToday->isEmpty())
                                <div class="text-center py-8">
                                    <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                    <p class="mt-2 text-sm text-gray-500">Semua anggota sibuk</p>
                                </div>
                            @else
                                <div class="space-y-2 max-h-80 overflow-y-auto">
                                    @foreach($freeUsersToday as $freeUser)
                                        <div class="p-2 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition">
                                            <div class="flex items-center gap-2">
                                                <div class="flex-shrink-0 w-8 h-8 bg-green-200 rounded-full flex items-center justify-center text-green-700 font-bold text-xs">
                                                    {{ strtoupper(substr($freeUser->name, 0, 1)) }}
                                                </div>
                                                <div class="flex-1 min-w-0">
                                                    <p class="text-xs font-semibold text-gray-900 truncate">{{ $freeUser->name }}</p>
                                                    <div class="flex flex-wrap gap-1 mt-1">
                                                        @foreach($freeUser->getRoleNames()->take(2) as $role)
                                                            <span class="px-1.5 py-0.5 bg-white border border-green-200 text-green-700 rounded text-xs">
                                                                {{ ucfirst($role) }}
                                                            </span>
                                                        @endforeach
                                                    </div>
